...
Vagrant.configure("2") do |config|
  ...
  (1..APPSRV_COUNT).each do |i|
    ...
    $scriptWebServer << "
    echo worker.worker#{i}.type=ajp13 >> /etc/httpd/conf/workers.properties
    echo worker.worker#{i}.host=server#{i} >> /etc/httpd/conf/workers.properties
    echo worker.worker#{i}.port=8009 >> /etc/httpd/conf/workers.properties"
  end
  $scriptWebServer << "
  echo worker.lb.balance_workers=#{BALANCE_WORKER_STRING.chomp(',')} >> /etc/httpd/conf/workers.properties 
